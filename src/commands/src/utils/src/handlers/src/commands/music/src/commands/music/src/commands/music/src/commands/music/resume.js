...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("resume")
    .setDescription("▶️ استمر في التشغيل"),
  ...
  async execute(interaction, client) {
    ...
    if (!queue.paused) return interaction.editReply({ embeds: [new EmbedBuilder().setColor(0xff9900).setDescription("⚠️ تشتغل أصلاً!").setFooter({ text: footerText })] });
    queue.resume();
    await interaction.editReply({ embeds: [new EmbedBuilder().setColor(voidColor).setDescription("▶️ استمر التشغيل!").setFooter({ text: footerText })] });
  },
};
